...
#[allow(unused)]
pub fn handle(mut source: impl Input, target: impl Input) -> impl Iterator<Item = String> {
    ...
    source
        .filter(move |line| {
            extract(&line, &source_column)
                .map(|field| !contains(field, &target_fields))
                .unwrap_or(false)
        })
}
...
mod tests {
    ...
    fn it_handles_fields() {
        ...
        assert_result(handle(s1, s2), vec!["Minerva;Macgonagal", "Hermione;Granger", "Ronald;Weasley"]);
    }
}
